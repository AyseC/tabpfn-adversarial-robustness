"""Transfer Attack Experiment - Iris Dataset"""
import numpy as np
import torch
import json
import warnings
...
print("="*80)
print("TRANSFER ATTACK EXPERIMENT - IRIS DATASET")
print("="*80)
...
for source_name, source_model in models.items():
    ...
    for target_name, target_model in models.items():
        if target_name == source_name:
            success_rate = 1.0  # Self-transfer is 100%
        else:
            successes = 0
...
# Summary
print(f"\n{'='*80}")
print("TRANSFER ATTACK RESULTS - IRIS")
print(f"{'='*80}")
...
with open('results/transfer_attack_iris.json', 'w') as f:
    json.dump(transfer_results, f, indent=2)
print(f"\n✓ Saved: results/transfer_attack_iris.json")
